...
提供响应式 CSS Grid 和列跨度能力，不与可访问性数据 grid 混淆。
...
|---|---|---|---|
| `columns` | `1…24 / responsive map` | 12 | 布局列数 |
| `gap` | `spacing token / responsive map` | 4 | token 键，4 对应配置值 |
| `align` | `start / center / end / stretch` | stretch | 对齐 |
| `GridItem.span` | `positive integer / responsive map` | 全列 | 不大于当前列数 |
| `GridItem.offset` | `nonnegative integer` | 0 | 超范围告警 |
...
实现明确的列数/跨度/间距，不同时强制百分比宽度；非法跨度告警并安全降级。

## 6. PC、移动端与可访问性

**移动端：**base 全宽，md 后多列；不隐藏重要内容以适配。

**键盘与可访问性：**布局网格不加 role=grid；保持 DOM 顺序。

遵循 [响应式规范](../docs/08-PC移动端响应式规范.md) 与 [无障碍规范](../docs/09-无障碍与键盘交互.md)。手势或 hover 如适用，必须有键盘/点击替代；不通过裁掉内容伪装兼容小屏。

## 7. 主题与样式合同

`--dui-grid-gap`、`--dui-grid-column-gap`、`--dui-grid-row-gap`。
...
| 测试 ID | 场景 | 结果要求 |
|---|---|---|
| `C06-EDGE-01` | 断点跨越 | React / Vue 分别记录结果与证据 |
| `C06-EDGE-02` | span 超界 | React / Vue 分别记录结果与证据 |
| `C06-EDGE-03` | RTL offset | React / Vue 分别记录结果与证据 |
| `C06-EDGE-04` | 长内容/嵌套网格 | React / Vue 分别记录结果与证据 |

共用基线另覆盖 `C06-BASE-*`、适用的 `CTRL-*`、`A11Y-*`、`MOBILE-*`、`THEME-*`、`SSR-*`。不适用项写明原因；未执行项写“未运行”，不能默认通过。两框架接受同一行为案例，但各自运行并保存证据。

## 9. 导出、依赖与交付

**命名导出：**`Grid`、`GridItem` 与对应类型；GridItem 响应式 span/offset 不能泄漏到 DOM。

**目标子路径：**`@your-scope/ui-react/grid`、`@your-scope/ui-vue/grid`。样式为同名 `/style.css` 子路径，配合框架包 `base.css`，或使用全量 `style.css`。纯逻辑入口的 CSS 可以为空但需明确产物策略，不能暴露指向不存在文件的 exports。只有完成验收的模块才进入稳定导出清单。
