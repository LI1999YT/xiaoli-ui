...
以一排选项选择一个值，适合视图模式或筛选，而不是通用路由Tabs。
...
|---|---|---|---|
| `value/defaultValue` | `K / null` | 首个可用项 | Vue modelValue |
| `options` | `{value,label,disabled?}[]` | [] | key唯一 |
| `size` | `sm / md / lg` | Provider | 尺寸 |
| `block` | `boolean` | false | 均分宽度 |
| `disabled` | `boolean` | false | 组禁用 |

## 3. 事件、实例与时序

onValueChange/update:modelValue；选择当前值不重复通知。

模型更新不因父 prop 回写而重复通知；可取消默认行为与清理边界遵循通用契约。对确实不存在模型的纯展示组件，不添加无意义的 change 事件。

## 4. 内容扩展与公开部位

option；icon；root,item,indicator,label。
...
选中指示器适应动态尺寸；不根据数组索引保存业务值；无可用项呈禁用空组。

## 6. PC、移动端与可访问性

**移动端：**长标签可滚动或换为Select，由业务选择；不无限缩小文字。

**键盘与可访问性：**采用radiogroup/radio模式，组名称；方向键；不声明不存在的tabpanel。

遵循 [响应式规范](../docs/08-PC移动端响应式规范.md) 与 [无障碍规范](../docs/09-无障碍与键盘交互.md)。手势或 hover 如适用，必须有键盘/点击替代；不通过裁掉内容伪装兼容小屏。

## 7. 主题与样式合同

`--dui-segmented-height`、`--dui-segmented-padding`、`--dui-segmented-radius`、`--dui-segmented-bg`、`--dui-segmented-indicator-bg`、`--dui-segmented-active-color`。
...
| 测试 ID | 场景 | 结果要求 |
|---|---|---|
| `C51-EDGE-01` | 0值 | React / Vue 分别记录结果与证据 |
| `C51-EDGE-02` | 禁用 | React / Vue 分别记录结果与证据 |
| `C51-EDGE-03` | 动态options | React / Vue 分别记录结果与证据 |
| `C51-EDGE-04` | 键盘 | React / Vue 分别记录结果与证据 |
| `C51-EDGE-05` | block与长标签 | React / Vue 分别记录结果与证据 |
| `C51-EDGE-06` | RTL | React / Vue 分别记录结果与证据 |

共用基线另覆盖 `C51-BASE-*`、适用的 `CTRL-*`、`A11Y-*`、`MOBILE-*`、`THEME-*`、`SSR-*`。不适用项写明原因；未执行项写“未运行”，不能默认通过。两框架接受同一行为案例，但各自运行并保存证据。

## 9. 导出、依赖与交付

**命名导出：**`Segmented` 与对应公共类型；必要子组件的命名在实施 API 审查时补全并同步两框架。

**目标子路径：**`@your-scope/ui-react/segmented`、`@your-scope/ui-vue/segmented`。样式为同名 `/style.css` 子路径，配合框架包 `base.css`，或使用全量 `style.css`。纯逻辑入口的 CSS 可以为空但需明确产物策略，不能暴露指向不存在文件的 exports。只有完成验收的模块才进入稳定导出清单。
