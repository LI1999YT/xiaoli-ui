...
|---|---|---|---|
| `title` | `string` | locale 暂无数据 | 主说明 |
| `description` | `string` | 未设置 | 原因/引导 |
| `size` | `compact / normal` | normal | 场景尺寸 |

## 3. 事件、实例与时序

无内置重新请求；actions 由业务传按钮。

模型更新不因父 prop 回写而重复通知；可取消默认行为与清理边界遵循通用契约。对确实不存在模型的纯展示组件，不添加无意义的 change 事件。

## 4. 内容扩展与公开部位

illustration；title；description；actions；root,illustration,title,description,actions。
...
支持无插画紧凑模式；不默认展示“请求失败”冒充空；插画不引入远程资源。

## 6. PC、移动端与可访问性

**移动端：**小容器压缩留白但保留动作；内容可换行。

**键盘与可访问性：**装饰图隐藏，有意义图有替代文本；actions 使用真正按钮。

遵循 [响应式规范](../docs/08-PC移动端响应式规范.md) 与 [无障碍规范](../docs/09-无障碍与键盘交互.md)。手势或 hover 如适用，必须有键盘/点击替代；不通过裁掉内容伪装兼容小屏。

## 7. 主题与样式合同

`--dui-empty-padding`、`--dui-empty-gap`、`--dui-empty-title-color`、`--dui-empty-description-color`、`--dui-empty-illustration-size`。
...
| 测试 ID | 场景 | 结果要求 |
|---|---|---|
| `C26-EDGE-01` | 无描述/无插画 | React / Vue 分别记录结果与证据 |
| `C26-EDGE-02` | 长文本 | React / Vue 分别记录结果与证据 |
| `C26-EDGE-03` | 动作可用 | React / Vue 分别记录结果与证据 |
| `C26-EDGE-04` | 小容器 | React / Vue 分别记录结果与证据 |
| `C26-EDGE-05` | 不误用错误状态 | React / Vue 分别记录结果与证据 |

共用基线另覆盖 `C26-BASE-*`、适用的 `CTRL-*`、`A11Y-*`、`MOBILE-*`、`THEME-*`、`SSR-*`。不适用项写明原因；未执行项写“未运行”，不能默认通过。两框架接受同一行为案例，但各自运行并保存证据。

## 9. 导出、依赖与交付

**命名导出：**`Empty` 与对应公共类型；必要子组件的命名在实施 API 审查时补全并同步两框架。

**目标子路径：**`@your-scope/ui-react/empty`、`@your-scope/ui-vue/empty`。样式为同名 `/style.css` 子路径，配合框架包 `base.css`，或使用全量 `style.css`。纯逻辑入口的 CSS 可以为空但需明确产物策略，不能暴露指向不存在文件的 exports。只有完成验收的模块才进入稳定导出清单。
