...
|---|---|---|---|
| `target` | `Element / 延迟函数` | 页面滚动容器 | 不写死window |
| `visibilityHeight` | `nonnegative number` | 400 | CSS px |
| `smooth` | `boolean` | true | reduced-motion关闭 |
| `label` | `string` | locale返回顶部 | 按钮名称 |
| `position` | `logical offset配置` | bottom-end | 避开safe-area |

## 3. 事件、实例与时序

onClick/@click；onVisibilityChange/@visibility-change；实例 scrollToTop()。

模型更新不因父 prop 回写而重复通知；可取消默认行为与清理边界遵循通用契约。对确实不存在模型的纯展示组件，不添加无意义的 change 事件。

## 4. 内容扩展与公开部位

icon；default；root,button,icon。
...
只观察指定容器；隐藏时不可聚焦；不强制将焦点移到body；使用者可配置返回内容标题焦点策略。

## 6. PC、移动端与可访问性

**移动端：**不遮TabBar/键盘；触控区域足够大。

**键盘与可访问性：**真实button有名称；键盘可用；减弱动画；回顶部后焦点位置明确。

遵循 [响应式规范](../docs/08-PC移动端响应式规范.md) 与 [无障碍规范](../docs/09-无障碍与键盘交互.md)。手势或 hover 如适用，必须有键盘/点击替代；不通过裁掉内容伪装兼容小屏。

## 7. 主题与样式合同

`--dui-back-top-size`、`--dui-back-top-radius`、`--dui-back-top-bg`、`--dui-back-top-shadow`、`--dui-back-top-offset-inline`、`--dui-back-top-offset-block`。
...
| 测试 ID | 场景 | 结果要求 |
|---|---|---|
| `C64-EDGE-01` | 阈值 | React / Vue 分别记录结果与证据 |
| `C64-EDGE-02` | 自定义容器 | React / Vue 分别记录结果与证据 |
| `C64-EDGE-03` | 隐藏无Tab | React / Vue 分别记录结果与证据 |
| `C64-EDGE-04` | 减少动画 | React / Vue 分别记录结果与证据 |
| `C64-EDGE-05` | 动态target | React / Vue 分别记录结果与证据 |
| `C64-EDGE-06` | 卸载 | React / Vue 分别记录结果与证据 |

共用基线另覆盖 `C64-BASE-*`、适用的 `CTRL-*`、`A11Y-*`、`MOBILE-*`、`THEME-*`、`SSR-*`。不适用项写明原因；未执行项写“未运行”，不能默认通过。两框架接受同一行为案例，但各自运行并保存证据。

## 9. 导出、依赖与交付

**命名导出：**`BackTop` 与对应公共类型；必要子组件的命名在实施 API 审查时补全并同步两框架。

**目标子路径：**`@your-scope/ui-react/back-top`、`@your-scope/ui-vue/back-top`。样式为同名 `/style.css` 子路径，配合框架包 `base.css`，或使用全量 `style.css`。纯逻辑入口的 CSS 可以为空但需明确产物策略，不能暴露指向不存在文件的 exports。只有完成验收的模块才进入稳定导出清单。
