...
|---|---|---|---|
| `size` | `sm / md / lg / CSS length` | md | 视觉尺寸 |
| `label` | `string` | locale 加载中 | 可访问状态 |
| `inline` | `boolean` | true | 行内或块级 |
| `visible` | `boolean` | true | 显式展示控制 |
...
可与 aria-busy 区域组合；不伪造百分比；减少动画时仍展示静态可辨识符号。

## 6. PC、移动端与可访问性

**移动端：**不默认全屏遮罩；占位尺寸稳定。

**键盘与可访问性：**role=status 或宿主提供一次状态说明；嵌入有名称 loading button 时避免重复 live region。

遵循 [响应式规范](../docs/08-PC移动端响应式规范.md) 与 [无障碍规范](../docs/09-无障碍与键盘交互.md)。手势或 hover 如适用，必须有键盘/点击替代；不通过裁掉内容伪装兼容小屏。

## 7. 主题与样式合同

`--dui-spinner-size`、`--dui-spinner-color`、`--dui-spinner-track-color`、`--dui-spinner-stroke-width`、`--dui-spinner-duration`。
...
| 测试 ID | 场景 | 结果要求 |
|---|---|---|
| `C24-EDGE-01` | 减弱/关闭动画 | React / Vue 分别记录结果与证据 |
| `C24-EDGE-02` | 标签可读 | React / Vue 分别记录结果与证据 |
| `C24-EDGE-03` | 嵌入按钮不重复播报 | React / Vue 分别记录结果与证据 |
| `C24-EDGE-04` | visible 切换 | React / Vue 分别记录结果与证据 |

共用基线另覆盖 `C24-BASE-*`、适用的 `CTRL-*`、`A11Y-*`、`MOBILE-*`、`THEME-*`、`SSR-*`。不适用项写明原因；未执行项写“未运行”，不能默认通过。两框架接受同一行为案例，但各自运行并保存证据。

## 9. 导出、依赖与交付

**命名导出：**`Spinner` 与对应公共类型；必要子组件的命名在实施 API 审查时补全并同步两框架。

**目标子路径：**`@your-scope/ui-react/spinner`、`@your-scope/ui-vue/spinner`。样式为同名 `/style.css` 子路径，配合框架包 `base.css`，或使用全量 `style.css`。纯逻辑入口的 CSS 可以为空但需明确产物策略，不能暴露指向不存在文件的 exports。只有完成验收的模块才进入稳定导出清单。
