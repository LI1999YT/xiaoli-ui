...
> 优先级：**P0** · 分类：布局 · 当前状态：**待实施**。这是开发与验收规格，不表示组件已实现、已测试或已发布。

## 1. 用途与范围

基于 Flexbox 实现可响应的排列，不承担业务交互。
...
|---|---|---|---|
| `direction` | `row / column / responsive map` | row | 响应式转列 |
| `align` | `start / center / end / stretch / baseline` | stretch | 垂直对齐 |
| `justify` | `start / center / end / between / around` | start | 主轴分布 |
| `gap` | `spacing token / responsive map` | 0 | 不接受无单位裸字符串猜测 |
| `wrap` | `boolean` | false | 显式换行 |
| `as` | `div / section / nav / main` | div | 有限标签 |

## 3. 事件、实例与时序

无自定义事件，原生 attributes 传根节点。
...
保持 DOM 阅读顺序，不用 CSS order 反转真实业务顺序；响应式 map 使用统一断点生成类。

## 6. PC、移动端与可访问性

**移动端：**窄屏换列、min-width:0 与长内容测试，避免子项撑破屏幕。

**键盘与可访问性：**不为布局添加 role；不靠视觉反转改变键盘顺序。

遵循 [响应式规范](../docs/08-PC移动端响应式规范.md) 与 [无障碍规范](../docs/09-无障碍与键盘交互.md)。手势或 hover 如适用，必须有键盘/点击替代；不通过裁掉内容伪装兼容小屏。

## 7. 主题与样式合同

`--dui-flex-gap`、`--dui-flex-row-gap`、`--dui-flex-column-gap`。
...
| 测试 ID | 场景 | 结果要求 |
|---|---|---|
| `C05-EDGE-01` | 响应式方向切换 | React / Vue 分别记录结果与证据 |
| `C05-EDGE-02` | 长内容不撑破 | React / Vue 分别记录结果与证据 |
| `C05-EDGE-03` | RTL | React / Vue 分别记录结果与证据 |
| `C05-EDGE-04` | 嵌套布局不制造额外边距 | React / Vue 分别记录结果与证据 |

共用基线另覆盖 `C05-BASE-*`、适用的 `CTRL-*`、`A11Y-*`、`MOBILE-*`、`THEME-*`、`SSR-*`。不适用项写明原因；未执行项写“未运行”，不能默认通过。两框架接受同一行为案例，但各自运行并保存证据。

## 9. 导出、依赖与交付

**命名导出：**`Flex` 与对应公共类型；必要子组件的命名在实施 API 审查时补全并同步两框架。

**目标子路径：**`@your-scope/ui-react/flex`、`@your-scope/ui-vue/flex`。样式为同名 `/style.css` 子路径，配合框架包 `base.css`，或使用全量 `style.css`。纯逻辑入口的 CSS 可以为空但需明确产物策略，不能暴露指向不存在文件的 exports。只有完成验收的模块才进入稳定导出清单。
