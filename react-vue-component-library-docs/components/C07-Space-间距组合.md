...
|---|---|---|---|
| `direction` | `horizontal / vertical` | horizontal | 排列 |
| `size` | `spacing token / responsive map` | 2 | 使用间距 token |
| `wrap` | `boolean` | false | 可换行 |
| `align` | `start / center / end / baseline` | center | 水平时适用 |
...
default；separator；root,item,separator。
...
空节点不留下多余分隔符；避免克隆子项并覆盖其 key/ref/props；明确 wrapper 结构，不承诺 fragment 完全无 DOM。

## 6. PC、移动端与可访问性

**移动端：**按钮组允许换行；小屏操作组可变为纵向。

**键盘与可访问性：**分隔装饰 aria-hidden；不改变 Tab 顺序。
...
| 测试 ID | 场景 | 结果要求 |
|---|---|---|
| `C07-EDGE-01` | 条件渲染空子项 | React / Vue 分别记录结果与证据 |
| `C07-EDGE-02` | 动态增删 | React / Vue 分别记录结果与证据 |
| `C07-EDGE-03` | 换行 | React / Vue 分别记录结果与证据 |
| `C07-EDGE-04` | 分隔符数量正确 | React / Vue 分别记录结果与证据 |

共用基线另覆盖 `C07-BASE-*`、适用的 `CTRL-*`、`A11Y-*`、`MOBILE-*`、`THEME-*`、`SSR-*`。不适用项写明原因；未执行项写“未运行”，不能默认通过。两框架接受同一行为案例，但各自运行并保存证据。

## 9. 导出、依赖与交付

**命名导出：**`Space` 与对应公共类型；必要子组件的命名在实施 API 审查时补全并同步两框架。

**目标子路径：**`@your-scope/ui-react/space`、`@your-scope/ui-vue/space`。样式为同名 `/style.css` 子路径，配合框架包 `base.css`，或使用全量 `style.css`。纯逻辑入口的 CSS 可以为空但需明确产物策略，不能暴露指向不存在文件的 exports。只有完成验收的模块才进入稳定导出清单。
